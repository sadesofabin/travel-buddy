...
const createComment = catchAsync(async (req, res) => {
  ...
  if (!locationId || !userId || !text) {
    return res.status(400).json({ message: 'locationId, userId, and text are required' });
  }
...
const addReply = catchAsync(async (req, res) => {
  ...
  if (!commentId || !userId || !text) {
    return res.status(400).json({ success: false, message: 'commentId, userId, and text are required' });
  }
...
const likeReply = catchAsync(async (req, res) => {
  ...
  if (!commentId || !replyId || !userId) {
    return res.status(400).json({ success: false, message: 'commentId, replyId, and userId are required' });
  }
...
const getCommentsByPlaceId = catchAsync(async (req, res) => {
  ...
  if (!comments.length) {
    return res.status(404).json({ success: false, message: 'No comments found for this place' });
  }
...
module.exports = { createComment, likeComment, addReply, likeReply, getCommentsByPlaceId };
